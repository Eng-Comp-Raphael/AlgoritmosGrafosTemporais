...
public class Algoritmo1 {
    ...
    // Executa o Algoritmo 1 conforme o pseudocódigo (Original para Terminal)
        ...
        for (Edge e : edgeStream) {
            ...
            if ((t + lambda <= tOmega) && (t >= tempoEmU) && (tempoEmU != INFINITO)){
                ...
                if (t + lambda < temposChegada.get(v)) {
                    ...
                    // Salvar de onde veio para onde foi
    ...
    // =================================================================
    // NOVO MÉTODO: Executa e devolve o resultado para a Web (JSON)
    // =================================================================
    public String executarParaWeb(List<String> nos, String origem, String destino, int tAlpha, int tOmega, List<Edge> stream) {
        this.edgeStream = stream;
        
        // Limpa os dados de execuções anteriores
        temposChegada.clear();
        predecessores.clear();
        ...
        for (Edge e : edgeStream) {
            ...
            if ((t + lambda <= tOmega) && (t >= tempoEmU) && (tempoEmU != INFINITO)) {
                if (t + lambda < temposChegada.getOrDefault(v, INFINITO)) {
                    temposChegada.put(v, t + lambda);
                    predecessores.put(v, u);
                }
            } else if (t >= tOmega) {
        ...
        // Monta o texto de resposta em formato JSON
        if (tempoFinal == null || tempoFinal == INFINITO) {
            return "{\"caminho\": \"Inalcançável\", \"tempo\": \"∞\"}";
        } else {
            String caminhoCompleto = reconstruirCaminho(destino, origem);
            return "{\"caminho\": \"" + caminhoCompleto + "\", \"tempo\": \"" + tempoFinal + "\"}";
        }
    }
}